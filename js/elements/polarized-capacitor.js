...
export class PolarizedCapacitor extends OhmElement {
    capacitance = 0.00001; // 10uF
    resistance = 1e12;
    ...
    draw(ctx) {
        ...
        ctx.beginPath();
        ctx.arc(this.x + 50, cy, 20, Math.PI * 5/6, Math.PI * 7/6);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(this.x + 15, cy - 10);
        ctx.lineTo(this.x + 15, cy - 5);
        ctx.moveTo(this.x + 12.5, cy - 7.5);
        ctx.lineTo(this.x + 17.5, cy - 7.5);
        ctx.stroke();
